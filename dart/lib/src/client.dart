...
class CrongeneratorValidationException implements Exception {
  final List<String> errors;

  CrongeneratorValidationException(this.errors);

  @override
  String toString() => 'CrongeneratorValidationException: ${errors.join("; ")}';
}
...
/// Cron Expression Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/crongenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [schedule] (required) - Natural language description of the schedule (max 200 characters) [maxLength: 200]
class CrongeneratorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'schedule': ValidationRule(type: 'string', required: true, maxLength: 200),
  };

  CrongeneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/crongenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CrongeneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CrongeneratorValidationException(errors);
    }
  }

  /// Execute a request to the Cron Expression Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<CrongeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CrongeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CrongeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CrongeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CrongeneratorException('Resource not found');
      } else {
        throw CrongeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CrongeneratorException) rethrow;
      throw CrongeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Cron Expression Generator API client.
class CrongeneratorException implements Exception {
  final String message;

  CrongeneratorException(this.message);

  @override
  String toString() => 'CrongeneratorException: $message';
}
